...
for row in fla_1:
    if "LRLL " + str(row["LRLL"]) in fla_4["Star ID"]:
        fla_4_match_index = int(np.where(row["LRLL"] == fla_4_lrll)[0])
...
fla1_litpertable = LitPeriodTable(
    ...
    fla_1["RAdeg"],
    fla_1["DEdeg"],
    np.array(fla1_pers).astype(float),
    short_name="F13",
    long_name="Flaherty et al. 2013",
)


def make_period_comparison_figure(verbose=True, saving=False):
    ...
    for _periods, _coords, _name, litpertable_list in zip(
        ...
        vprint(f"{_name}")
        vprint(f"{np.sum(np.isfinite(_periods['Period']))} periodic objects")
        vprint("")
        ...
        with mpl.rc_context(
            ...
            for litpertable, symbol, ms in zip(litpertable_list, symbols, ms_list):
                ...
                vprint(f"  Matches to {litpertable.long_name}: {np.sum(sep_constraint)}")
                vprint(
                ...
                vprint(f"  Closest match: {np.min(d2d).to(u.arcsec):.2f}")
                vprint(
                    f"  Farthest used match: {np.max(d2d[sep_constraint]).to(u.arcsec):.2f}"
                )

                try:
                    litpertable.period = litpertable.period.filled(np.nan)
                ...
                vprint(f"  Number of overlapping periods: {n_overlap}")
            ...
            vprint(
                f"Total number of sources (periodic and not) in {_name}: {len(_periods)}"
            )

            is_periodic = np.isfinite(_periods["Period"])
            n_is_periodic = np.sum(is_periodic)
            vprint(f"Number of periodics: {n_is_periodic}")
            ...
            vprint("******** ********** ******** ****** ********")
            vprint(f"Number of **new** periodics we found: {np.sum(new_periodics)}")
            vprint("******** ********** ******** ****** ********")

            missed_periodics = ~np.isfinite(_periods["Period"]) & was_periodic

            vprint("")
            vprint(f"Number of known periodics we missed: {np.sum(missed_periodics)}")
            ...
            first_legend = ax.legend(
                handles=period_plots,
                loc="upper left",
                title=f"{_name} period comparisons\n",
            )
            ...
            ax.plot([0.01, 100], [0.01 / 2, 100 / 2], "k--", lw=0.1)
            ...
            if saving:
                filename = os.path.join(figure_export_path, f"Figure6_{_name.split()[0]}")
...
if __name__ == "__main__":
    ...
    make_period_comparison_figure(verbose=False)
